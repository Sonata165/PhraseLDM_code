...
model_fp = '/data1/longshen/Results/AccGenResults/diffusion_prior/phr_latent/full_song/baseline_lencond_from_bar_permute/tb_logs/version_1/checkpoints/step_step=100000.ckpt'
...
if 'last' in os.path.basename(model_fp):
    ...
    kstep = int(step) // 1000
    step_str = f'{kstep}k'
save_dir = f'/data1/longshen/Results/AccGenResults/test_outputs/ldm/PhrLDM/{model_name}/from_{pt}/step_{step_str}'
...
for i in range(n_song):
    ...
    visualize_ssm_for_song_bar(z_trunc, save_dir=ssm_dir, filename=f'{n_bar}bar_{i}_mem{mem_rate:.2f}_memorized{memorized_samples[i]}.png')
